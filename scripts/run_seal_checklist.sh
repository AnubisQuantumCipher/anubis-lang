...
# The correspondence map is a pure source/formal claim surface: every cited theorem and path must
# resolve, and the TCB list must be non-empty. It does not execute Anubis, so exempt only this gate
# from the traced-pin invocation requirement instead of adding a vacuous token binary call.
