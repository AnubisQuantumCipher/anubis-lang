...
anubis_guard_sync_tree() {
  ...
  if ! "${rsh_argv[@]}" "$remote_host" \
    "set -u; root=\"\$HOME/$remote_rel\"; q=\$(mktemp -d \"\$HOME/.anubis-sync-quarantine.XXXXXX\") || exit 42; for spec in .git/worktrees=.git__worktrees out=out implementer/a_plus_audit_run=implementer__a_plus_audit_run .claude/worktrees=.claude__worktrees .hermes=.hermes adversary=adversary vm/exports=vm__exports scratchpad=scratchpad; do rel=\${spec%%=*}; name=\${spec#*=}; src=\"\$root/\$rel\"; if test -e \"\$src\" || test -L \"\$src\"; then mv \"\$src\" \"\$q/\$name\" || exit 42; fi; done; if test -e \"\$root/vm\" || test -L \"\$root/vm\"; then test -d \"\$root/vm\" && test ! -L \"\$root/vm\" || exit 42; else mkdir \"\$root/vm\" || exit 42; fi; if test -e \"\$root/vm/pins\" || test -L \"\$root/vm/pins\"; then test -d \"\$root/vm/pins\" && test ! -L \"\$root/vm/pins\" || exit 42; else mkdir \"\$root/vm/pins\" || exit 42; fi; setopt NULL_GLOB 2>/dev/null || :; for p in \"\$root/vm/pins/\"* \"\$root/vm/pins/\".[!.]* \"\$root/vm/pins/\"..?*; do if ! test -e \"\$p\" && ! test -L \"\$p\"; then continue; fi; base=\${p##*/}; case \"\$base\" in anubis-[0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f][0-9a-f]) if test -f \"\$p\" && test ! -L \"\$p\"; then continue; fi ;; esac; mv \"\$p\" \"\$q/pin__\$base\" || exit 42; done; rm -f -- \"\$root/.DS_Store\" || exit 42"; then
