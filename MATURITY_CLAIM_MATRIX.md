...
| Full 15 unified gates | REAL | `scripts/audit_unified.sh` → 15/15 PASS, 0 FAIL, 0 SKIP (337 tests) | `bash scripts/audit_a_plus.sh` (or `audit_unified.sh`) | Reconciled 2026-07-11; canonical runner. `scripts/audit_a_plus.sh` is now a thin fail-closed alias that delegates here (previously a 4-gate stub) — guarded by `audit_a_plus_front_door_runs_the_real_gate_suite_not_a_stub` + `unified_gate_suite_is_fail_closed`. G10 was silently red (PCA cold-verify coupled to tool version); fixed 2026-07-11 |
...
| Claim | Status | Evidence | Command | Notes |
|-------|--------|----------|---------|-------|
| Unified gate suite (one command, all gates, stranger can run) | REAL | `scripts/audit_unified.sh` — 15 gates (G1-G15), JSON report | `bash scripts/audit_a_plus.sh` (or `audit_unified.sh`) → 15/15 PASS, 0 FAIL, 0 SKIP | G10 prove ran green after the cold-verify fix; G15 skips only when no `examples/feel/` dir |
