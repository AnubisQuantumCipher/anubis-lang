...
| Program-wide Safe/Research/Exploit aggregation | under Command (2026-07-31 bounded activated epoch) | `program_mode` recursive join; one Build/Run/Prove/Repl consent+VZ predicate; `safe_mode_program_gate.rs`; `research_policy_callers.rs`; `formal/Anubis/ModeAggregation.lean` | final source-current guest `anubis-run-35251`, 22/22 named gates, offensive guest `anubis-offensive-gate-43849`, 34/34, and `out/phase1_finalization_51f4_r2_20260731T230000Z/` | Highest privilege wins independent of source order and nesting; research requires explicit consent and the VZ marker before lowering/execution; ordinary Safe remains accepted. Lean proves the abstract lattice only; Rust tests cover implementation traversal. Pin `anubis-51f4a964347a`; frozen source tree `b3b5bfd8…`. The external receipt proves the required VM/offensive/921-row-diff/20-row host-seal predicate, and its read-only review records `APPROVE` with no blocking finding. That activates bounded Phase 1 for the exact frozen epoch; it does not make the dirty epoch landed, released, shipped, or universally sound |
...
| Legacy local release diagnostic | under Command (scripted; not publishable) | `scripts/build_release_candidate.sh` + bounded local artifacts | `bash scripts/build_release_candidate.sh --metal-reference ... --require-metal` | Exercises a useful subset but is not commit-bound, not the 29-gate seal, and cannot authorize a tag/release; use the Phase-1 proof-first transaction for publication |
...
| Claim | Status | Evidence | Command | Notes |
|-------|--------|----------|---------|-------|
| Runtime probe capability evidence | under Command | `runtime-probe.json`, `RUNTIME_PROBE.md`, `MANIFEST.sha256` | `anubis runtime-probe --json --evidence --metal-reference $ANUBIS_RISC0_METAL_REFERENCE --out out/a16_runtime_probe` | Captures host/toolchain/RISC0/Metal ref hashes+commit/dirty; explicitly not proof execution |
...
| Offensive gate | under Command (2026-07-31 bounded technical epoch) | `out/phase1_offensive_51f4_postmetrics_final_20260731T185000Z/{report.json,isolation.json,export_manifest.json,offensive_verdict.json,teardown_status.txt}`; pin `anubis-51f4a964347a` | `ANUBIS_OFFENSIVE_GATE_VM_MEM=5120 bash scripts/run_offensive_platform_gate.sh --out …` plus independent strict revalidation | **34/34**, `isolation=tart-disposable-guest`, doctor cases PASS, manifest identity MATCH, strict validator PASS with no errors, guest `anubis-offensive-gate-41607` torn down; the earlier 2026-07-30 mismatched bundle remains RED history. Dirty tree is unlanded and unshipped |
...
deterministic, source-derived bounded verdict (mode, tier, parse/typecheck/solver, `verdict`) — and
...
|-------|--------|----------|---------|
| Claim block emitted into every evidence bundle | under Command | PCA v2 `pca.json` (`pca_version`, `source_sha256`, `mode`, `tier`, `parse_ok`, `typecheck_ok`, `solver_obligations`, `solver_all_discharged`, `verdict`); no independent taint-clean field is claimed while `docs/CLAIMS.md` item 21 is open | `anubis check FILE --evidence --out d && cat d/evidence-*/pca.json` |
...
| `verify` re-derives the bounded claim from source and cross-checks | under Command | `verify_pca` re-parses/typechecks/runs the bounded solver obligation pass over the bundle's `source.anubis` and requires it to equal strict PCA v2 `pca.json`; unknown fields, missing `pca.json`, and v1's retired `taint_clean` schema fail closed | `anubis verify d/evidence-*` |
...
| Tamper/schema gate: source, claim, and retired-schema attacks fail closed | under Command | 19/19 at `out/phase1_pca_postmetrics_final_20260731T190000Z` — strict v2 shape; clean/cold/signature controls; tampered source/claim; rehashed v1, v2-plus-retired-field, unknown-field v2, and missing-PCA poisons all reject. The tracked minimum remains 18 and the run reports the one-check excess explicitly | `bash scripts/run_pca_gate.sh --out out/phase1_pca_postmetrics_final_20260731T190000Z` |

**PCA v2 scope (honest):** `tier` is `"checked"`: parsing, the bounded implementation typecheck,
and its solver-obligation pass completed. It is not a total taint/noninterference theorem and not a
full T1/T2 assurance tier. Bundles may carry an Ed25519 PCA signature and a separately reverified ZK
receipt; neither upgrades the bounded semantic claim. Historical suite counts below remain dated.
...
| Claim | Status | Evidence | Command | Notes |
|-------|--------|----------|---------|-------|
| Unified gate suite (one command, all gates, stranger can run) | under Command | `scripts/audit_unified.sh` — current 29-gate roster (G1-G29), JSON report | `bash scripts/audit_a_plus.sh --out <new-root>`; historical 2026-07-24 receipt was 15/15 before G16-G29 were added | A current full claim requires 29/29 with no FAIL/SKIP/EXTERNAL and operator-run disposable Tart/VZ evidence |
...
| CI runs the bounded hosted front door (not a weak subset or a false full seal) | under Command | `.github/workflows/ci.yml` execs `scripts/audit_unified.sh --profile hosted` on `macos-latest`; guarded by `ci_workflow_keeps_vz_and_metal_out_of_hosted_ci` | `cargo test -p anubis-compiler ci_workflow_keeps_vz_and_metal_out_of_hosted_ci`; then inspect the exact GitHub run for the pushed SHA | Hosted success is 28 PASS plus exactly G9 EXTERNAL, with G14 limited to its 5-check non-executing isolation witness. Full VZ/offensive and require-Metal work is approved operator-run evidence outside public CI; no persistent self-hosted runner is authorized. |
...
## Runtime evidence identity hardening (2026-07-29)

| Claim | Status | Evidence | Command | Notes |
|-------|--------|----------|---------|-------|
| Runtime probe/plan hash identity v2 | SOURCE-VERIFIED; fresh pin + VM seal pending | `tools/anubis/src/runtime_reference_hash.rs`; runtime-probe/runtime-plan schema 1.1; tool suite 351/351 | `cargo test -p anubis --bin anubis`; then `bash scripts/run_seal_checklist.sh` | Default in-repo reference hashes only the vendor tree; explicit references hash the declared root excluding `.git`, `target`, and `.DS_Store`. Length framing prevents concatenation collisions; traversal/read errors, symlinks, special files, unreadable paths, and unequal consecutive complete walks fail readiness. Reports explicitly state that two complete walks must match and that this is not atomic against adversarial flip-back mutation. This row does **not** claim VM closure. |
