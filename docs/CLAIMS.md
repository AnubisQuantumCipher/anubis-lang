...
|---|---|---|
| **Security fixtures** | Lead gate **336/336 PASS**. Live disk inventory **336** `.anb`; **published red list EMPTY** (0 `EXPECT: FAIL` still check-PASS this pass) | Green ≠ no bugs. Re-enumerate command below. |
...
| **Native authoritative** | **PASS over 936 files, 0 mismatches** (current corpus; the earlier 2026-07-29 ratchet raised 906 → 916) | `bash scripts/run_native_authoritative_gate.sh` |
...
17. **`build` and `run` research CONSENT gap — SOURCE/VZ-CLOSED on the bounded dirty tree;
    ...
    epoch, compiler library **771/771**, language **259/259**, security **336/336**, stdlib
    ...
    a failed receipt, not promoted: security **336/336**, language **259/259**, stdlib fail-closed
    **104/104**; the current native-authoritative corpus is **936 files**, while that failed receipt
    ...
    **213 builtins**, while check/run parity and the documentation-coverage floor were RED. Phase 1
    ...
    to a whole-tree seal**: native-authoritative enumerated **936 files** from the live disk while the
...
21. **SIX items published as CLOSED are OPEN — 30 true accepts with runtime witnesses, survived
    ...
    **Row 8 — FIXED for annotated containers, clean-VM sealed, and landed in `03210603`
    ...
    | security corpus | **336/336** — includes the ten annotated list/map/generic/parameter fixtures |
    | language corpus | **259/259** |
    | stdlib fail-closed | **104/104**, `timed_out=0` |
    | native-authoritative | current corpus **936 files**; this W1 receipt graded 916 files, 0 mismatches, 0 disagreements |
...
[receipt-scope: pre-fix-only; head: 0f407853; authority: none]
    ...
    **Bounded W1 receipt (2026-07-29, commit `03210603`).** This closes only the annotated
    ...
    | security | **336/336 PASS** |
    | language | **259/259 PASS** |
    | stdlib fail-closed | **104/104 PASS** |
    | native-authoritative | current corpus **936 files**; this W1 receipt graded 916 files, 0 mismatches |
...
| Safe taint enforcement | security **336/336** (lead) / red list empty live; original D1–D4 fixture shapes reject; taint selfhost **0 disagreements** | **PARTIAL as total** — item 21 reopens broader composition/carrier routes; green = **no KNOWN defects**, not no defects. Stdlib **104/104** |
