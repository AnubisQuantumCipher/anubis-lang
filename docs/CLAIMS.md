...
| if-expr-built containers seed `field_closures` (incl. nested `Stmt::If` + let-inner) | **CLAIMED 2026-07-25** | `collect_container_closures` walks `Expr::If`/`Match`/`Block`; `parse_expr_block` nested bare `if` is `Stmt::If` — seeded via `collect_block_container_closures` + local-let tail resolve; unit `nested_container_closure_application_fails_closed` (flat if, nested Stmt::If, let-inner, mid-bind, clean accept); release probes `nested_if_true`/`nested_if_as_let` REJECT secret | Residual: full PC-join; deeper escaping-closure forms beyond this table |
...
| Non-exportable linear capabilities (static + interproc sealedness) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc:** return-mint seeds caller non_exportable; formals that reach export sinks without peel cause call-site EXPORT; units `nonexportable_return_mint_*` + `nonexportable_param_print_is_export` + peel-before-call accept; `cargo test -p anubis-compiler --lib capability` | Residual: Keychain/SE hardware isolation **not claimed**; closure-captured caps; HO rebind; peel-of-param (params unknown-provenance for `cap_export`) |
...
| Effect-derived entitlement / sandbox profile | **CLAIMED 2026-07-25** | `anubis entitlements <file.anb>`; `package::entitlements` derive + seal `entitlement_profile.json` + `program.entitlements` plist; re-derive on PCA verify (`ANUBIS_ENTITLEMENT_DRIFT`); unit `cargo test -p anubis-compiler entitlement` → 7 passed | Residual: OS enforcement only after codesign (`apple_enforced_claim: false`); **Keychain/SE-backed linear caps permanently residual** (language TCB stops at static non-exportable + causal spend — no hardware item bind claimed); path-level sandbox rules **not claimed** |
