...
**Status vocabulary (2026-07-24):** freestanding **REAL** / production-grade / fully proven
stamps are banned. A claim is either (a) re-runnable with a command + observation, (b) sealed
under a dated artifact path that is not re-run this session, (c) **partial** with the gap named,
or (d) **not claimed**.
...
| Wrap-safety VCs (AoRTE-lite) + CEX possible fix | **CLAIMED 2026-07-25** | On modelable ints: auto wrap-safety for `+`/`-` and **var×const `*`**; fail `ANUBIS_WRAP_RISK` with pretty CEX + paste-ready `requires(...)`; opt-out `ANUBIS_WRAP_SAFETY=0`; unit `cargo test -p anubis-compiler --lib wrap_safety` → 5+; see [`SPARK_VS_ANUBIS.md`](SPARK_VS_ANUBIS.md) | Residual: **free×free `*` wrap-safety not checked** (hang risk on free smul encodings — skipped, not silently proved); free `ensures(x*y)` can still be slow under native-authoritative |
| Implicit secret→public (PC) + explicit secret→public (Safe) | **CLAIMED 2026-07-25** | `ANUBIS_IMPLICIT_FLOW` + `ANUBIS_SECRET_TO_PUBLIC` including **method** formals; fixtures `implicit_flow_*` + `secret_to_public_*`; unit `secret_to_public_store_and_return_rejected`; security fixtures **149/149** | Residual: full PC-label join (not Jif-total); untyped formals still interproc |
| Symbolic-index secret-capturing closure application | **CLAIMED 2026-07-25** | `arr[idx](…)` with non-literal `idx` fail-closed when container holds secret/taint-capturing element (j1 twin of `let g = arr[i]`); unit `symbolic_index_secret_capturing_list_application_fails_closed`; clean symbolic still accepts | Residual: full PC-join; deeper HO residual forms stay fail-closed/docs |
| Verified causal capability spend | **CLAIMED 2026-07-25** | Verified privileged builtins require a **live matching-kind** token at the effect (`cap_acquire("kind")` → effect spends it); wrong kind / no token → `ANUBIS_EFFECT_UNAUTHORIZED`; double-spend → `ANUBIS_CAPABILITY_REUSE`; fixtures `cap_causal_*`; unit `middle::capability::tests::*` | Safe remains declaration-gated (`uses`); interprocedural / HO cap flow residual |
| Non-exportable linear capabilities (static) | **CLAIMED 2026-07-25** | `cap_acquire_nonexportable("kind")` mints `non_exportable`; token-as-arg to `print`/`send`/`http_*`/`write_file` → `ANUBIS_CAPABILITY_EXPORT`; causal spend still allowed when token is not an arg; well-formed `cap_export(c, "reason")` peels; unit `cargo test -p anubis-compiler capability` → 35 passed | Residual: Keychain/SE hardware isolation **not claimed**; interproc non_exportable; real Keychain item bind |
...
| Native authoritative **division** (`bvsdiv`/`bvsrem`) | **partial CLAIMED 2026-07-25** | Nonneg + power-of-two `/` `%` → proven `bvlshr`/`bvand` (requires `x>=0`); general/non-pow2 still deferred (`/3` danger fixture) |
...
| VZ apply Softnet CIDR from DNS-pinned allow-host | **CLAIMED 2026-07-25** | With `softnet` on PATH: `--allow-host` → tart `--net-softnet-block=0.0.0.0/0` + `--net-softnet-allow=<ip>/32`; mode `hostname-softnet`; without softnet → `hostname-policy-staged` host-only fallback; unit `cargo test -p anubis vz_apply` | Residual: DNS rebinding after pin; not L7 hostname; Keychain; live tart boot not in gate |
| Effect-derived entitlement / sandbox profile | **CLAIMED 2026-07-25** | `anubis entitlements <file.anb>`; `package::entitlements` derive + seal `entitlement_profile.json` + `program.entitlements` plist; re-derive on PCA verify (`ANUBIS_ENTITLEMENT_DRIFT`); unit `cargo test -p anubis-compiler entitlement` → 7 passed | Residual: OS enforcement only after codesign (`apple_enforced_claim: false`); Keychain/SE-backed linear caps **not claimed**; path-level sandbox rules **not claimed** |
