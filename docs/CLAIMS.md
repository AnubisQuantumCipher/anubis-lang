...
| Non-exportable linear capabilities (static + interproc + closure capture + aggregate peel + LetPattern move + cond/source seal) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** return-mint + exporting formals; **closure capture** of Live NE into export sinks (definition-site export-seal: straight-line, `if` **cond+arms**, **loop bodies + header exprs** — `while`/`for`/`loop`/`while let` cond/source/invariant + research/exploit/hybrid; enum/map/struct nesting); **value-position aggregate peel** (`print((s,1))` / lists / struct/map/enum/if/match tails); **LetPattern positional MOVE**; units `nonexportable_*` incl. loop body, **if/while cond, for-source, while-let expr** + clean duals; `cargo test -p anubis-compiler --lib capability` (60) | Residual: Keychain/SE hardware isolation **not claimed**; **store-then-index** after aggregate consume accept-biased; peel-of-param; deep HO rebind beyond sealed table |
