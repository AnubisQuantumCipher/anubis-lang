...
| Non-exportable linear capabilities (static + interproc + closure capture + aggregate peel + LetPattern move) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** return-mint + exporting formals; **closure capture** of Live NE into export sinks (definition-site export-seal walk through straight-line, `if`, and **loop bodies** — `while`/`for`/`loop`/`while let`/`research`/`exploit`/`hybrid`); **value-position aggregate peel** (`print((s,1))` / `print([s])` / struct/map/enum/if/match tails); **LetPattern positional MOVE** (`let (a,_)=(s,1); print(a)`); units `nonexportable_*` incl. loop, tuple/list arg, LetPattern move + clean duals; `cargo test -p anubis-compiler --lib capability` (54) | Residual: Keychain/SE hardware isolation **not claimed**; **store-then-index** after aggregate consume (`let arr=[s]; print(arr[0])`) accept-biased (unknown provenance never invents NE); peel-of-param; deep HO rebind beyond sealed table |
