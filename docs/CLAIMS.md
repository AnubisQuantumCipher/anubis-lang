...
| Non-exportable linear capabilities (shared visitor + store-then-project) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** return-mint + **exporting formals** (body + headers via `for_each_export_child`); **closure capture** export-seal; **store-then-project** sealed (`let arr=[s]; print(arr[0])` / map get — container_ne tracking); aggregate peel + LetPattern MOVE; dual matrix units; `cargo test -p anubis-compiler --lib capability` (70) | Residual: Keychain/SE hardware isolation **not claimed**; peel-of-param; deep HO rebind / symbolic container project beyond concrete store-then-index |
