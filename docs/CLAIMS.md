...
| Non-exportable linear capabilities (shared visitor + store-then-project + interproc container stores) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** formals + headers; **closure capture** export-seal; **store-then-project** (`container_ne` + formal container map): literal store, free/method `push`/`insert` (incl. nested place root), place-write, **project-bind**, **if/match/block** init; **lambda free-NE** seed; **interproc formal-container mutation** (`container_stores` summary + fixpoint compose: `stash(arr,s){push(arr,s)}; print(arr[0])` EXPORT); dual matrix units; `cargo test -p anubis-compiler --lib middle::capability::tests` | Residual: Keychain/SE **not claimed**; peel-of-param; deep HO rebind; ambient interproc **causal spend** (effect auth still direct-builtins-only); HO rebind of container stores beyond formal-index summary |
