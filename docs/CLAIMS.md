...
| Non-exportable linear capabilities (static + interproc + closure capture) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** return-mint + exporting formals; **closure capture** of Live NE into export sinks (definition-site export-seal walk through straight-line, `if`, and **loop bodies** — `while`/`for`/`loop`/`while let`/`research`/`exploit`/`hybrid`); units `nonexportable_return_mint_*`, `nonexportable_param_print_is_export`, `nonexportable_closure_capture_print_is_export`, `nonexportable_returned_closure_capture_print_is_export`, `nonexportable_closure_capture_print_in_{while,for,loop,while_let}_is_export`, send twin; `cargo test -p anubis-compiler --lib capability` | Residual: Keychain/SE hardware isolation **not claimed**; HO rebind of caps beyond capture-into-export-sink; peel-of-param (params unknown-provenance for `cap_export`) |
