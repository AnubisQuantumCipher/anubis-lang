...
| Non-exportable linear capabilities (shared visitor + store-then-project + push/insert + interproc/lambda container) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** formals + headers; **closure capture** export-seal; **store-then-project** (`container_ne` + formal container map): literal store, free/method `push`/`insert` (incl. nested place root), place-write, **project-bind** (`let x = arr[0]; print(x)`), **if/match/block** value-position container init; **lambda free-NE** export-seal seed; dual matrix units; `cargo test -p anubis-compiler --lib middle::capability::tests` → **78 passed** | Residual: Keychain/SE hardware isolation **not claimed**; peel-of-param; deep HO rebind beyond sealed table; ambient interproc causal spend + ambient interproc container mutation without sink in callee |
