...
| VZ apply network fail-closed (hostname staged) | **CLAIMED 2026-07-25** | Dual of mounts: host-only refuses `--allow-host`/`--allow-open-nat`; `net.send` defaults to host-only (not open NAT); `--allow-host` DNS-pins + records (`hostname-policy-staged`); `--allow-open-nat` explicit residual; unit `cargo test -p anubis vz_apply` → 12 passed; gate net-deny | Residual: tart does **not** enforce per-hostname (native egress pump / Softnet); Keychain still residual |
