...
| Nested container closure application (`outer[0][0]`, `b.fs[i]`) | **CLAIMED 2026-07-25** | Nested Index/FieldAccess CallExpr resolves `field_closures` via flatten keys (`"0.0"`, `"fs.0"`); symbolic segment fail-closes any capturing lambda under root; unit `nested_container_closure_application_fails_closed`; clean nested still accepts | Residual: if-expr-built containers may not seed field_closures (fail-open under-report until seeded); full PC-join not claimed |
