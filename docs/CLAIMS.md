...
| Nested container closure application (`outer[0][0]`, `b.fs[i]`, bind twin) | **CLAIMED 2026-07-25** | Nested Index/FieldAccess CallExpr + **bind** (`let g = outer[i][0]; g(0)`) resolve via flatten keys / fail-closed any symbolic segment in the chain; unit `nested_container_closure_application_fails_closed` (apply + bind); clean nested still accepts | Residual: if-expr-built containers may not seed field_closures (fail-open under-report until seeded); full PC-join not claimed |
