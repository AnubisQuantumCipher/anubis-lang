...
| Non-exportable linear capabilities (shared export-reachable visitor: lambda seal + interproc formals) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** return-mint + **exporting formals** (body + **if/while cond, for-source, while-let expr** headers via same control-flow surface as lambda seal); **closure capture** definition-site export-seal; shared `for_each_export_child` + dual matrix units (lambda NE × formal NE × {body, if-cond, while-cond, for-source, while-let expr} + clean duals); aggregate peel + LetPattern MOVE; `cargo test -p anubis-compiler --lib capability` (67) | Residual: Keychain/SE hardware isolation **not claimed**; **store-then-index** after aggregate consume accept-biased; peel-of-param; deep HO rebind beyond sealed table |
