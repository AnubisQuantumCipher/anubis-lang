...
| Non-exportable linear capabilities (shared visitor + store-then-project + interproc container stores) | **CLAIMED 2026-07-25** | Local mint + export sinks → `ANUBIS_CAPABILITY_EXPORT`; causal spend without token-as-arg OK; `cap_export` peels; **interproc** formals + headers; **closure capture** export-seal; **store-then-project** + **interproc formal-container mutation**; dual matrix units; `cargo test -p anubis-compiler --lib middle::capability::tests` | Residual: peel-of-param; deep HO; ambient interproc causal spend |
| Keychain / Secure Enclave bind for NE caps (macOS runtime) | **partial CLAIMED 2026-07-25** | `cap_acquire_nonexportable` runtime prefers Keychain item (`__anubis_cap_ne_kc:…`) on macOS; `ANUBIS_KEYCHAIN_SE=1` tries SE-resident EC key handle (`__anubis_cap_ne_se:…`); soft fallback `__anubis_cap_ne_soft:…`; `keychain_se_probe` → 0/1/2; unit `keychain_se_probe_and_ne_acquire_run`; gate `bash scripts/run_keychain_se_gate.sh`; entitlement derive adds `keychain-access-groups` + `com.apple.developer.secure-enclave` when NE present (`apple_enforced_claim: false`) | Residual: **unsigned CLI soft-fallback**; production SE isolation needs codesign + access groups + operator attestation — not claimed by soft mint alone |
