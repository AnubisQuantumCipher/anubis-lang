...
| VZ apply mount posture fail-closed | **CLAIMED 2026-07-25** | Engagement `--dir` filtered by proven mount posture: `none` → `ANUBIS_APPLY_MOUNT_DENIED`; `read-only` forces `:ro`; unit `cargo test -p anubis vz_apply` → 7 passed; gate mount-deny control | Residual: Softnet/hostname allow-list; live tart boot not required for gate; SE/Keychain still residual |
