...
- Escaping-closure class: **map-entry application closed 2026-07-22** (`map_entry_closure_application_is_enforced`); **flat symbolic-index application closed 2026-07-25**; **nested container application closed 2026-07-25** (`nested_container_closure_application_fails_closed` — `outer[0][0]` / `b.fs[i]`). Residual: if-expr-built containers may not seed `field_closures` (named fail-open under-report); SH ⊆ Rust elsewhere
