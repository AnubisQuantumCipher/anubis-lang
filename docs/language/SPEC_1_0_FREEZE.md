...
- Non-exportable capability **export-seal**: shared `for_each_export_child`; dual matrix; aggregate peel + LetPattern MOVE; **store-then-project** via `container_ne` (literal store, free/method push/insert, place-write → index/field/print container EXPORT). Residual: Keychain/SE permanent; peel-of-param; deep HO beyond sealed table
