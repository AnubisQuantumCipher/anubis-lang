...
- Non-exportable capability **export-seal**: definition-site walk of NE-capturing lambdas into export sinks closed for straight-line, nested `if`, and **loop bodies** (`while`/`for`/`loop`/`while let` + research/exploit/hybrid stmt arms) — units under `middle::capability::tests::nonexportable_closure_capture_print_in_*` (commit `18979d6`). Residual: Keychain/SE hardware bind permanent; deep HO rebind of caps beyond capture-into-export-sink
