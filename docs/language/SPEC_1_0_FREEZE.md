...
- Non-exportable capability **export-seal**: shared visitor; store-then-project; interproc formal + container_stores fixpoint; lambda free-NE. **Keychain/SE bind (CLAIMED signed path):** soft / Keychain (`kc:`) / optional SE (`se:`); `compile_sign_and_run_source` codesigns with Development identity + safe CLI entitlements; unit requires `kc:` under Development; gate `run_keychain_se_gate.sh`. Residual: App Store/notarized .app packaging; restricted SE provisioning UX; map/struct-field linear-closure containers. Peel-of-param **closed**; ambient interproc causal spend **closed**; deep HO + list-container linear closures **closed** (units `linear_closure_*`)
