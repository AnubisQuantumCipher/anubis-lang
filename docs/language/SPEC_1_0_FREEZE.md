...
- Non-exportable capability **export-seal**: shared `for_each_export_child`; dual matrix; aggregate peel + LetPattern MOVE; **store-then-project** via `container_ne`; **interproc formal** export + **formal-container mutation summary** (`container_stores` + fixpoint); **lambda free-NE** export-seal seed. **Keychain/SE runtime bind (partial):** macOS `cap_acquire_nonexportable` may mint `kc:` / `se:` handles (`keychain_se_runtime.inc.rs`); soft fallback always; entitlement derive for keychain-access-groups + secure-enclave when NE present. Residual: production SE under signed identity still needs codesign (not claimed by soft mint); peel-of-param; deep HO; ambient interproc causal spend
