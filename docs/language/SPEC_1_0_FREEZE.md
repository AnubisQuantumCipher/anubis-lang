...
- Non-exportable capability **export-seal**: shared `for_each_export_child`; dual matrix; aggregate peel + LetPattern MOVE; **store-then-project** via `container_ne` (literal store, free/method push/insert incl. nested place root, place-write, project-bind `let x = arr[i]`, if/match/block value-position init → index/field/print EXPORT); **interproc formal** export + **formal-container mutation summary** (`container_stores` + fixpoint compose through callees: stash helper push/insert then caller project EXPORT); **lambda free-NE** export-seal seed. Residual: Keychain/SE permanent; peel-of-param; deep HO beyond sealed table; ambient interproc causal spend (effect auth still direct-builtins-only)
- Implicit secret-PC assignment to public locals **and public returns under secret PC**: **REJECTED in Safe mode 2026-07-25** (`ANUBIS_IMPLICIT_FLOW`); residual is full PC-label join propagation (not claimed Jif-total) — assignment matrix already covers if/while/for/match/nested if
