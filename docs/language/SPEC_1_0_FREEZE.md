...
- Implicit secret-PC assignment to public locals: **REJECTED in Safe mode 2026-07-25** (`ANUBIS_IMPLICIT_FLOW`) under `if`/`while`/`while let`/`for` (secret bounds), statement- and value-position `match`/`if let`, and secret match guards; residual is full PC-label join propagation (not claimed Jif-total)
