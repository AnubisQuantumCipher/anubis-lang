...
- Non-exportable capability **export-seal**: shared `for_each_export_child`; dual matrix; aggregate peel + LetPattern MOVE; **store-then-project** via `container_ne` (literal store, free/method push/insert incl. nested place root, place-write, project-bind `let x = arr[i]`, if/match/block value-position init → index/field/print EXPORT); **interproc formal** container store via `formals_reaching_export_sink` container map; **lambda free-NE** export-seal seed. Residual: Keychain/SE permanent; peel-of-param; deep HO beyond sealed table; ambient interproc causal spend + ambient interproc container mutation without sink in callee
