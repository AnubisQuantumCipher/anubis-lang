...
- Escaping-closure class: **map-entry application closed 2026-07-22** (`map_entry_closure_application_is_enforced`); **flat symbolic-index application closed 2026-07-25**; **nested container application + bind twin + intermediate mid-bind closed 2026-07-25**; **if-expr-built containers (incl. nested bare `if` as `Stmt::If` inside `parse_expr_block` + let-inner tails) seed `field_closures` closed 2026-07-25** (`nested_container_closure_application_fails_closed`). Residual: full PC-label join not claimed; SH ⊆ Rust elsewhere; deeper escaping-closure forms beyond the sealed table
