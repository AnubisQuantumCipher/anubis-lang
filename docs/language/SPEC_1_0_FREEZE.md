...
- Non-exportable capability **export-seal**: definition-site walk of NE-capturing lambdas into export sinks closed for straight-line; nested `if` **cond + arms**; **loop bodies + header exprs** (`while`/`for`/`loop`/`while let` cond/source/invariant + research/exploit/hybrid); enum/map/struct nesting under seal walk; **value-position aggregate peel** at export sinks; **LetPattern positional MOVE** of NE. Residual: Keychain/SE hardware bind permanent; **store-then-index** after aggregate consume (accept-biased sealedness); deep HO rebind beyond sealed table
