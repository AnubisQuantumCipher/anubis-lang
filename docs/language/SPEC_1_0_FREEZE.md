...
- Non-exportable capability **export-seal**: shared `for_each_export_child` visitor powers both lambda definition-site seal and interproc formal export summary — **same** control-flow surface (If.cond + arms, While/For/WhileLet/Loop headers + bodies, research/exploit/hybrid). Dual matrix: lambda NE capture and formal NE arg × body/if-cond/while-cond/for-source/while-let-expr. Aggregate peel + LetPattern MOVE. Residual: Keychain/SE permanent; **store-then-index** accept-biased; deep HO rebind beyond sealed table
