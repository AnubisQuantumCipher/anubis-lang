...
- Non-exportable capability **export-seal**: shared `for_each_export_child` visitor (lambda seal + interproc formals, same headers); dual matrix; aggregate peel + LetPattern MOVE; **store-then-project** (`container_ne`: list/map/struct store of Live NE → index/field project to export sink is EXPORT). Residual: Keychain/SE permanent; peel-of-param; deep HO / symbolic container project beyond concrete store-then-index
