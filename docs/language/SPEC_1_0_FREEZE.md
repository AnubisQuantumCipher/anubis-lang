...
- Escaping-closure class: **map-entry application closed 2026-07-22** (`map_entry_closure_application_is_enforced`); **flat symbolic-index application closed 2026-07-25**; **nested container application + bind twin + intermediate mid-bind closed 2026-07-25** (`nested_container_closure_application_fails_closed` — direct `outer[0][0](…)` / `b.fs[i](…)`, `let g = outer[i][0]; g(…)`, and `let mid = outer[0]; mid[0](…)` with `field_closures` re-key / symbolic first-segment union). Residual: if-expr-built containers may not seed `field_closures` (named fail-open under-report); SH ⊆ Rust elsewhere
