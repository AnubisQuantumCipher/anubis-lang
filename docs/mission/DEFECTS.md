...
| RT-STRIDX | S1 | RUNTIME: native `p["key"]` on a struct returns its FIRST declared field whatever the key (`index_get`: a non-numeric string key parses to index 0), so `p["pub_n"]` printed the secret `k` | `rv11_struct_string_index_*`, `rv12_strindex_unknown_base` | checker made conservative in 8424dc0c; **runtime fix is the next unit** |
