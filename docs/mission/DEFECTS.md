...
| id | sev | defect | evidence | state |
|---|---|---|---|---|
| F-PARSE-1 | S1 | malformed source (`let x = );`, `let x = 1 + ;`, unexpected tokens in `requires`/`assert`, `"${1+}"`) gets `check` rc 0 and JSON `verdict:"pass"`; the parser makes a placeholder node without an error (frontend/mod.rs:4042) | probes, review 02 | **fixed** `d5ed1d72`: unexpected token and mid-expression EOF are diagnostics; matrix `parse_*` |
| F-PARSE-2 | S3 | no parser recursion-depth limit; deeply nested input stack-overflows the compiler (exit 134) | probes, review 02 | **fixed** `d5ed1d72`: `MAX_PARSE_DEPTH`=256 guard on primary/statement/pattern/interpolation paths; 200k-deep input is a diagnostic, rc 1, not SIGABRT |
...
| L-LINT-1 | S2 | the workspace clippy gate (`-D warnings`) fails on Linux: 7 errors (macOS-only imports/methods unused off macOS, a needless `return` in a non-macOS cfg block, two newer-clippy lints). Invisible because CI is macOS-only (A-CI-1) | cargo clippy on aarch64 Linux, rustc 1.98.1 | fixed `a94537f7` (macOS side not compiled locally; macOS CI is the witness) |
| L-RUN-2X | S4 | `anubis run` off macOS typechecked the whole program, discarded the result, then `run_anubis_source` typechecked it again: every Linux run did the full analysis twice | tools/anubis/src/main.rs run_anubis_source_signed | fixed `a94537f7` |
...
| IFC2-REVIEW-2 | S1-S3 | review round 2 of IFC v2 (pin `anubis-ifc2-v8e`, the fixed version before its final landing changes): 75 historical confirmed findings; category and verifier details remain in the recovered inventory | [recovered report, programs and verifier outcomes](../evidence/IFC2_ROUND2_2026-09-26/REPORT.md); [leak registration and focused checker witness](../evidence/ROUND2_REGISTRATION_2026-09-26.md) | **open**; all historical leak carriers are registered, but most current verdicts and required guest replays remain unmeasured; min/max remains unintegrated |
| M-ARM-BINDER | S1/S3 | Statement-match post-filter unions binder names from all arms and drops a violated `requires` from an arm that has no such binder; its same-arm treatment also refuses valid bound calls as undecided | `r2arm_sibling_*`, `r2arm_same_binder_*` and valid/dead/guard controls; [pinned checker witnesses](../evidence/ARM_BINDER_REGISTRATION_2026-09-26/README.md) | **open**; sibling cases silently accept, while same-binder and valid reachability controls remain undecided |
| M-CALL-POSITION | S1/S3 | Contracted calls evaluated in statement `match` scrutinees/guards and `if let` scrutinees, including assignment RHS inside a guard, emit no effective `requires` obligation; guard reachability and arm-local facts need precision | `r2arm_contract_call_scrutinee`, `r2arm_contract_call_guard`, `r2arm_iflet_scrutinee_call`, `r2arm_guard_assign_rhs_*`, direct twins and valid controls; [pinned checker witnesses](../evidence/ARM_BINDER_REGISTRATION_2026-09-26/README.md) | **open**; violated carriers silently accept, direct twins disprove, valid controls must remain accepted; distinct from sibling-binder filtering |
| IFC2-PROOF-BOOL-KIND | S1 | `proof_commit_bool` was modeled as returning its input shape although native and guest return a scalar; this skipped secret egress in a later method fallback argument | `ifc2r2_proof_bool_*`; [source-bound evidence](../evidence/PROOF_BOOL_RESULT_2026-09-26/README.md) | **implemented** b835b94b; 7 focused library tests pass; CLI build succeeds and focused full-Safe/IFC2 checks report 14 PASS; full workspace/matrix/guest/platform gates remain unverified |
| IFC2-MINMAX-KEY | S1/S3 | comparator keys select the incumbent passed to later callbacks; the candidate's repeatability fragment still newly refuses fixed comparison, negation and helper keys | `r2p_06`, `r2_fn_L1`; [withheld candidate and required failing controls](../evidence/MINMAX_REPEATABILITY_2026-09-26/README.md) | **open**; no min/max compiler patch integrated; 16 passed, 3 failed in the expanded candidate suite |
