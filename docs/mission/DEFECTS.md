...
| M-CALL-POSITION | S1 | Contracted calls evaluated in statement `match` scrutinees/guards and `if let` scrutinees emit no effective `requires` obligation | `r2arm_contract_call_scrutinee`, `r2arm_contract_call_guard`, `r2arm_iflet_scrutinee_call` and direct twins; [pinned checker witnesses](../evidence/ARM_BINDER_REGISTRATION_2026-09-26/README.md) | **open**; carriers silently accept, direct twins disprove; distinct from sibling-binder filtering |
