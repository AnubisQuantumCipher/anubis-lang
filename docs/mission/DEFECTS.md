...
| M-ARM-BINDER | S1 | Statement-match post-filter unions binder names from all arms and drops a violated `requires` from an arm that has no such binder; the obligation's assumption variables create a second same-arm hazard | `r2arm_*` matrix controls; [pinned checker witness](../evidence/ROUND2_REGISTRATION_2026-09-26.md) | **open**; violated calls accepted while direct and renamed controls disprove them, and a satisfied control remains accepted |
