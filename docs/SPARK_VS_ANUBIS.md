...
- **Ops covered:** signed `+` and `-` always (when operands modelable); `*` for
  **variable × constant** (cheap closed-form range) and **free×free** via **offline
  interval product** under requires (no SMT smul — those encodings hung native QF_BV).
  Unbounded free×free → `ANUBIS_WRAP_RISK` + joint-bound possible fix; bounded factors
  whose four corner products fit i64 → proved (obligation not emitted).
